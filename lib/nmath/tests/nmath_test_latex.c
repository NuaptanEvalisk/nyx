...
{
  setbuf(stdout, NULL);
  printf("NMath Test Program, LaTeX Rendering.\n");
  ...
  globals.latex_exe_path = "/usr/bin/pdflatex";
  ...
  err_proc(e);
  clock_gettime(CLOCK_MONOTONIC, &t1);
  printf("Precompiled LaTeX prelude. [%.2f ms]\n", get_ms(t0, t1));
  ...
  clock_gettime(CLOCK_MONOTONIC, &t0);
  e = nmath_render_quality(formula, nmath_formula_display, &bitmap, runtime);
  err_proc(e);
  clock_gettime(CLOCK_MONOTONIC, &t1);
  printf("Rendered NMath formula using LaTeX backend. [%.2f ms]\n",
         get_ms(t0, t1));
